...
import Test.SmallCheck.Series (NonNegative (..), Positive (..), Serial (..))
import Test.Tasty
...
type family ConcatMap (w :: Type -> Constraint) (cs :: [Type]) :: Constraint where
  ConcatMap w '[] = ()
  ConcatMap w (c ': cs) = (w c, ConcatMap w cs)

type family Matrix (as :: [Type -> Constraint]) (w :: Type -> Type) (bs :: [Type]) :: Constraint where
  Matrix '[] w bs = ()
...
testIntegralProperty ::
  forall wrapper bool.
  (TestableIntegral wrapper, SC.Testable IO bool, QC.Testable bool) =>
  String -> (forall a. (Integral a, Bits a, Show a) => wrapper a -> bool) -> TestTree
testIntegralProperty name f =
  testGroup
    name
...
testIntegral2Property ::
  forall wrapper1 wrapper2 bool.
  (TestableIntegral wrapper1, TestableIntegral wrapper2, SC.Testable IO bool, QC.Testable bool) =>
  String -> (forall a1 a2. (Integral a1, Integral a2, Bits a1, Bits a2, Show a1, Show a2) => wrapper1 a1 -> wrapper2 a2 -> bool) -> TestTree
testIntegral2Property name f =
  testGroup
    name
